...
const [postError, setPostError]= useState("");
  useEffect(() => {
    axios.get("http://localhost:3001/webscraper")
      .then((res) => {
        setisLoading(false);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await axios.post("http://localhost:3001/webscraper", {url})
      ...
      <div className="webScraperContainer">
        ...
        <form onSubmit={handleSubmit}>
            <span>
              ...
              <input type="text" value={url} onChange={(e) => setUrl(e.target.value)} name="url" id="url" placeholder="Enter your url"/>
            </span>
          ...
          <div className="scraperItems">
            ...
            {
              scraperData && scraperData.map((data) => 
              data.scraperData.map((res) => <div key={res.meta} className="scraperItem">
